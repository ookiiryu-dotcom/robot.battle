...
export class Enemy extends Player {
  ...
  getShootInterval() {
    return this.typeConfig.shootInterval ?? this.config.enemy.shootInterval;
  }
  ...
  applyEnemyLook() {
    ...
    if (this.typeId === "close") {
      this.bodyRig.scale.multiplyScalar(1.14);
      this.shoulderBar.scale.x *= 1.24;
      this.chestCore.scale.multiplyScalar(1.28);
    }
  ...
  updateEnemy(dt, player, skills, callbacks = {}) {
    ...
    let moving = false;
    if (this.typeId === "close") {
      moving = distance > this.config.enemy.meleeRange
        ? this.moveToward(player, this.config.enemy.closeMoveSpeed, dt)
        : false;

      if (distance <= this.config.enemy.meleeRange && this.meleeTimer <= 0) {
        if (player.takeDamage(this.config.enemy.meleeDamage)) callbacks.onPlayerDeath?.();
        this.meleeTimer = this.config.enemy.meleeCooldown;
      }
    } else if (this.typeId === "mid") {
      if (distance > 28) {
        moving = this.moveToward(player, this.config.enemy.midMoveSpeed, dt);
      }
      this.shootTimer -= dt;
      if (this.shootTimer <= 0) {
        skills.shootFrom(this, yaw, "enemy", 0xffb86b, {
          damage: this.typeConfig.projectileDamage,
          speed: this.typeConfig.projectileSpeed,
          lifetime: this.typeConfig.projectileLifetime
        });
        this.shootTimer = this.getShootInterval();
      }
    } else {
      if (distance < this.config.enemy.farPreferredDistance) {
        moving = this.moveToward(player, this.config.enemy.farRetreatSpeed, dt, true);
      }
      this.shootTimer -= dt;
      if (this.shootTimer <= 0) {
        skills.shootFrom(this, yaw, "enemy", 0xc084fc, {
          damage: this.typeConfig.projectileDamage,
          speed: this.typeConfig.projectileSpeed,
          lifetime: this.typeConfig.projectileLifetime
        });
    ...
    this.updateAnimation(moving, this.typeId === "close");
  }
}
