...
export class SkillController {
  ...
  createProjectileMesh(color, options = {}) {
    ...
    if (options.shape === "cross") {
      ...
      const barA = new THREE.Mesh(geometry, material.clone());
  ...
  applyBeamDamage(beam, player, enemies, callbacks = {}) {
    const targets = beam.team === "player"
    ...
    for (const hitTarget of targets) {
      ...
      if (hitTarget.takeDamage(damage, { team: beam.team, type: "beam" })) {
        if (beam.team === "player") callbacks.onEnemyDeath?.(hitTarget);
        else callbacks.onPlayerDeath?.();
      }
    }
  }
  ...
  updateHeldShield(player, yaw, active) {
    ...
    this.heldShield.collider.onBlock = (attack) => {
      if (attack.team !== "enemy" || typeof player.absorbGuardShield !== "function") return;
  ...
  projectileHitsTarget(projectile, player, enemies) {
    ...
    if (projectile.team === "enemy") {
      if (this.segmentHitsBody(start, end, player, player.radius + projectileRadius + hitPadding, verticalPadding + projectileRadius)) {
        return player;
      }
      return null;
    }
    ...
    return enemies.find((enemy) => {
      return enemy.hp > 0
        && this.segmentHitsBody(start, end, enemy, enemy.radius + projectileRadius + hitPadding, verticalPadding + projectileRadius);
    }) || null;
  }
  ...
  applyExplosionDamage(position, projectile, player, enemies, callbacks = {}) {
    ...
    const targets = projectile.team === "enemy" ? [player] : enemies.filter((enemy) => enemy.hp > 0);
    let hitAny = false;
  ...
  update(dt, player, enemies, callbacks = {}) {
    ...
    for (const slash of [...this.slashes]) {
      ...
      const hitTarget = slash.team === "enemy"
        ? (this.segmentHitsBody(slash.previousPosition, slash.mesh.position, player, player.radius + slash.hitRadius, slashVerticalPadding) ? player : null)
        : enemies.find((enemy) => enemy.hp > 0 && this.segmentHitsBody(
      ...
      if (blocked) {
        ...
        if (hitTarget.takeDamage(slash.damage, { team: slash.team, type: "slash" })) {
          if (hitTarget === player) callbacks.onPlayerDeath?.();
          else callbacks.onEnemyDeath?.(hitTarget);
        }
        this.createExplosion(slash.mesh.position);
        this.removeItem(this.slashes, slash);
      } else if (expired) {
