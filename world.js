...
export class World {
  ...
  createReusableGeometry() {
    ...
    this.ringGeometry = new THREE.TorusGeometry(2.3, 0.025, 8, 60);
  ...
  createChunk(cx, cz) {
    const { chunkSize, chunkSegments } = this.config.world;
  ...
  createCoverWalls(group, cx, cz) {
    const { chunkSize, coverWallMinCount, coverWallRandomCount } = this.config.world;
  ...
  updateChunks(playerPosition) {
    const { chunkSize, viewDistance } = this.config.world;
